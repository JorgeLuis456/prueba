...
import { Reseña, Usuario, Plato } from "../types";

type ReseñaModalProps = {
  resena: Reseña | null;
  setResenaEditar: (r: Reseña | null) => void;
...
export default function ReseñaModal({
  ...
  const [nuevaResena, setNuevaResena] = useState<Reseña>(
    resena || {
      id: 0,
      id_plato: 0,
      id_usuario: "",
  ...
  const guardarResena = () => {
    if (!nuevaResena.id_plato || !nuevaResena.id_usuario)
      return alert("Selecciona usuario y plato");
    setResenas((prev) =>
      resena
        ? prev.map((r) => (r.id === resena.id ? nuevaResena : r))
        : [...prev, { ...nuevaResena, id: prev.length + 1 }]
    );
    cerrarConAnimacion();
  };
  ...
  return (
    ...
    >
      ...
      >
        ...
        <select
          value={nuevaResena.id_usuario}
          onChange={(e) =>
            setNuevaResena({ ...nuevaResena, id_usuario: e.target.value })
          }
          style={{
            width: "100%",
            padding: "10px",
        ...
        <label style={{ fontWeight: 600, color: "#3B200B" }}>Plato:</label>
        <select
          value={nuevaResena.id_plato}
          onChange={(e) =>
            setNuevaResena({ ...nuevaResena, id_plato: Number(e.target.value) })
          }
          style={{
            width: "100%",
            padding: "10px",
        ...
        <label style={{ fontWeight: 600, color: "#3B200B" }}>Puntuación:</label>
        <input
          type="number"
          min={1}
          max={5}
        ...
        <label style={{ fontWeight: 600, color: "#3B200B" }}>Comentario:</label>
        <textarea
